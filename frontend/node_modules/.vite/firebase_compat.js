...
var FirebaseAppImpl = class {
  ...
  _getService(name22, instanceIdentifier = DEFAULT_ENTRY_NAME) {
    var _a2;
    this._delegate.checkDestroyed();
    const provider = this._delegate.container.getProvider(name22);
  ...
  _removeServiceInstance(name22, instanceIdentifier = DEFAULT_ENTRY_NAME) {
    this._delegate.container.getProvider(name22).clearInstance(instanceIdentifier);
  }
...
function createFirebaseNamespaceCore(firebaseAppImpl) {
  ...
  function removeApp(name22) {
    delete apps[name22];
  }
  function app(name22) {
    name22 = name22 || DEFAULT_ENTRY_NAME;
    if (!contains(apps, name22)) {
  ...
  function getApps() {
    return Object.keys(apps).map((name22) => apps[name22]);
  }
  ...
  function useAsService(app2, name22) {
    if (name22 === "serverAuth") {
      return null;
    }
    const useService = name22;
    return useService;
  }
  return namespace;
}
...
function repoRerunTransactionQueue(repo, queue2, path) {
  ...
  const callbacks = [];
  let events = [];
  const txnsToRerun = queue2.filter((q2) => {
    return q2.status === 0;
  });
...
function push(parent2, value) {
  ...
  const name22 = nextPushId(now);
  const thennablePushRef = child(parent2, name22);
  const pushRef = child(parent2, name22);
  let promise;
  if (value != null) {
...
function set2(ref3, value) {
  ...
  const deferred = new Deferred();
  repoSetWithPriority(ref3._repo, ref3._path, value, null, deferred.wrapCallback(() => {
  }));
  return deferred.promise;
}
...
function enableLogging(logger6, persistent) {
  enableLogging$1(logger6, persistent);
}
...
var name9 = "@firebase/database-compat";
var version9 = "0.1.5";
...
var Query = class {
  ...
  startAt(value = null, name22) {
    validateArgCount("Query.startAt", 0, 2, arguments.length);
    return new Query(this.database, query(this._delegate, startAt(value, name22)));
  }
  startAfter(value = null, name22) {
    validateArgCount("Query.startAfter", 0, 2, arguments.length);
    return new Query(this.database, query(this._delegate, startAfter(value, name22)));
  }
  endAt(value = null, name22) {
    validateArgCount("Query.endAt", 0, 2, arguments.length);
    return new Query(this.database, query(this._delegate, endAt(value, name22)));
  }
  endBefore(value = null, name22) {
    validateArgCount("Query.endBefore", 0, 2, arguments.length);
    return new Query(this.database, query(this._delegate, endBefore(value, name22)));
  }
  equalTo(value, name22) {
    validateArgCount("Query.equalTo", 1, 2, arguments.length);
    return new Query(this.database, query(this._delegate, equalTo(value, name22)));
  }
  toString() {
...
function initStandalone({ app, url, version: version22, customAuthImpl, namespace, nodeAdmin = false }) {
  setSDKVersion(version22);
  ...
  return {
    instance: new Database2(repoManagerDatabaseFromApp(app, authProvider, void 0, url, nodeAdmin), app),
    namespace
  };
}
...
var name10 = "@firebase/firestore-compat";
...
var Firestore = class {
  ...
  namedQuery(name22) {
    return Pa(this._delegate, name22).then((expQuery) => {
      if (!expQuery) {
        return null;
      }
...
var UserDataWriter = class extends xh {
  ...
  convertReference(name22) {
    const key = this.convertDocumentKey(name22, this.firestore._databaseId);
...
function registerFirestore(instance) {
  ...
  instance.registerVersion(name10, version10);
}
...
function httpsCallable$1(functionsInstance, name22, options) {
  return (data) => {
    return call(functionsInstance, name22, data, options || {});
  };
}
...
async function call(functionsInstance, name22, data, options) {
  const url = functionsInstance._url(name22);
  data = encode2(data);
  const body = { data };
  const headers = {};
...
function registerFunctions(fetchImpl, variant) {
  ...
  registerVersion(name11, version11, variant);
  registerVersion(name11, version11, "esm2017");
}
function connectFunctionsEmulator(functionsInstance, host, port) {
  connectFunctionsEmulator$1(getModularInstance(functionsInstance), host, port);
}
function httpsCallable(functionsInstance, name22, options) {
  return httpsCallable$1(getModularInstance(functionsInstance), name22, options);
}
registerFunctions(fetch.bind(self));

// node_modules/@firebase/functions-compat/dist/index.esm2017.js
var name12 = "@firebase/functions-compat";
var version12 = "0.1.9";
var FunctionsService2 = class {
  ...
  httpsCallable(name22, options) {
    return httpsCallable(this._delegate, name22, options);
  }
...
registerFunctions2();
firebase.registerVersion(name12, version12);
...
function registerMessagingInWindow() {
  ...
  registerVersion(name13, version13);
  registerVersion(name13, version13, "esm2017");
}
...
var name14 = "@firebase/messaging-compat";
var version14 = "0.1.9";
...
firebase.registerVersion(name14, version14);
...
function registerStorage() {
  ...
  registerVersion(name15, version15, "");
  registerVersion(name15, version15, "esm2017");
}
registerStorage();
...
var name16 = "@firebase/storage-compat";
...
function registerStorage2(instance) {
  ...
  instance.registerVersion(name16, version16);
}
registerStorage2(firebase);

// node_modules/@firebase/performance/dist/esm/index.esm2017.js
var name17 = "@firebase/performance";
var version17 = "0.5.5";
var SDK_VERSION3 = version17;
...
var Api = class {
  ...
  getEntriesByName(name22) {
    if (!this.performance || !this.performance.getEntriesByName) {
      return [];
    }
    return this.performance.getEntriesByName(name22);
  }
  getTimeOrigin() {
...
function isValidCustomAttributeName(name22) {
  if (name22.length === 0 || name22.length > MAX_ATTRIBUTE_NAME_LENGTH) {
    return false;
  }
  const matchesReservedPrefix = RESERVED_ATTRIBUTE_PREFIXES.some((prefix) => name22.startsWith(prefix));
  return !matchesReservedPrefix && !!name22.match(ATTRIBUTE_FORMAT_REGEX);
}
...
function sendLog(resource, resourceType) {
  if (!logger5) {
    logger5 = transportHandler(serializer);
  }
  logger5(resource, resourceType);
}
...
function isValidMetricName(name22, traceName) {
  if (name22.length === 0 || name22.length > MAX_METRIC_NAME_LENGTH) {
    return false;
  }
  return traceName && traceName.startsWith(OOB_TRACE_PAGE_LOAD_PREFIX) && oobMetrics.indexOf(name22) > -1 || !name22.startsWith(RESERVED_AUTO_PREFIX);
}
...
var Trace = class {
  constructor(performanceController, name22, isAuto = false, traceMeasureName) {
...
function trace(performance, name22) {
  performance = getModularInstance(performance);
  return new Trace(performance, name22);
}
...
function registerPerformance() {
  ...
  registerVersion(name17, version17);
  registerVersion(name17, version17, "esm2017");
}
registerPerformance();
...
var name18 = "@firebase/performance-compat";
var version18 = "0.1.5";
function registerPerformanceCompat(firebaseInstance) {
  ...
  firebaseInstance.registerVersion(name18, version18);
}
...
var name19 = "@firebase/remote-config";
var version19 = "0.3.4";
...
var CachingClient = class {
  constructor(client, storage2, storageCache, logger6) {
    this.client = client;
    this.storage = storage2;
...
function registerRemoteConfig() {
  ...
  registerVersion(name19, version19);
  registerVersion(name19, version19, "esm2017");
  function remoteConfigFactory2(container, { instanceIdentifier: namespace }) {
    ...
    const logger6 = new Logger(name19);
    logger6.logLevel = LogLevel.ERROR;
    ...
    const cachingClient = new CachingClient(retryingClient, storage2, storageCache, logger6);
    const remoteConfigInstance = new RemoteConfig(app, cachingClient, storageCache, storage2, logger6);
...
var name20 = "@firebase/remote-config-compat";
var version20 = "0.1.5";
function registerRemoteConfigCompat(firebaseInstance) {
  ...
  firebaseInstance.registerVersion(name20, version20);
}
...
firebase.registerVersion(name21, version21, "compat");
